...
from test import bls_train, bls_train_tcga
...
def train_a_epoch(model, train_data, all_data, patient_and_time, patient_sur_type, batch_size, optimizer, epoch,
    ...
    #
    # print(f"模型的参数量为: {total_params/1000000}M")
    lbl_pred_each = None
    ...
    for i_batch, id in enumerate(train_data):
        ...
        # flops, params = profile(model, (graph, use_type_eopch, use_type_eopch, mask, args.mix))
...
def main(args):
    ...
    if cancer_type == 'mydataset':
        ...
        patients = joblib.load('E:\Project\HGCN-main\dataset1\patients.pkl')
        ...
        seed_fit_split = joblib.load('E:\Project\HGCN-main\dataset1\mydata_split.pkl')
    elif cancer_type == 'lusc':
        patients = joblib.load('E:\Project\HGCN-main\TCGA\LUSC\lusc_patients.pkl')
        sur_and_time = joblib.load('E:\Project\HGCN-main\TCGA\LUSC\lusc_sur_and_time.pkl')
        if model_type == 'hgcn':
            all_data = joblib.load('E:\Project\HGCN-main\TCGA\LUSC\lusc_data.pkl')
        seed_fit_split = joblib.load('E:\Project\HGCN-main\TCGA\LUSC\lusc_split.pkl')
        ...
        patients = joblib.load('E:\Project\HGCN-main\TCGA\LUAD\luad_patients.pkl')
        sur_and_time = joblib.load('E:\Project\HGCN-main\TCGA\LUAD\luad_sur_and_time.pkl')
        if model_type == 'hgcn':
            all_data = joblib.load('E:\Project\HGCN-main\TCGA\LUAD\luad_data.pkl')
        seed_fit_split = joblib.load('E:\Project\HGCN-main\TCGA\LUAD\luad_split.pkl')
    elif cancer_type == 'ucec':
        patients = joblib.load(r'E:\Project\HGCN-main\TCGA\UCEC\ucec_patients.pkl')
        sur_and_time = joblib.load(r'E:\Project\HGCN-main\TCGA\UCEC\ucec_sur_and_time.pkl')
        if model_type == 'hgcn':
            all_data = joblib.load(r'E:\Project\HGCN-main\TCGA\UCEC\ucec_data.pkl')
        seed_fit_split = joblib.load(r'E:\Project\HGCN-main\TCGA\UCEC\ucec_split.pkl')
    elif cancer_type == 'kirc':
        patients = joblib.load('E:\Project\HGCN-main\TCGA\KIRC\kirc_patients.pkl')
        sur_and_time = joblib.load('E:\Project\HGCN-main\TCGA\KIRC\kirc_sur_and_time.pkl')
        if model_type == 'hgcn':
            all_data = joblib.load('E:\Project\HGCN-main\TCGA\KIRC\kirc_data.pkl')
        seed_fit_split = joblib.load('E:\Project\HGCN-main\TCGA\KIRC\kirc_split.pkl')
    elif cancer_type == 'lihc':
        patients = joblib.load('E:\Project\HGCN-main\TCGA\Lihc\lihc_patients.pkl')
        sur_and_time = joblib.load('E:\Project\HGCN-main\TCGA\Lihc\lihc_sur_and_time.pkl')
        if model_type == 'hgcn':
            all_data = joblib.load('E:\Project\HGCN-main\TCGA\Lihc\lihc_data.pkl')
        seed_fit_split = joblib.load('E:\Project\HGCN-main\TCGA\Lihc\lihc_split.pkl')
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patients, kf_label):
            ...
            if args.if_fit_split:
                ...
                # print(all_data[id].x_img.shape)
                # print(patient_sur_type[id])
                val_data = seed_fit_split[n_fold - 1][1]
                test_data = seed_fit_split[n_fold - 1][2]

                def bls(all_data, train_data, val_data, test_data, patients):
                ...
                def bls_tcga(all_data, train_data, val_data, test_data, patients):
                    node_cli = bls_train_tcga(all_data, train_data, val_data, test_data, patients)
                    for id in patients:
                        all_data[id].x_cli = torch.tensor(node_cli[id], dtype=torch.float).to(device)
                        all_data[id].to(device)
                    return all_data
                ...
                if model_type == 'my':
                    if cancer_type == 'mydataset':
                        ...
                        all_data = joblib.load(r'E:\Project\HGCN-main\gendata\all_100.pkl')
                        all_data = bls(all_data, train_data, val_data, test_data, patients)
                    elif cancer_type == 'mydataset1':
                        all_data = joblib.load('E:\Project\HGCN-main\dataset1/alldata.pkl')
                        all_data = bls(all_data, train_data, val_data, test_data, patients)
                    elif cancer_type == 'lihc':
                        all_data = joblib.load('E:\Project\HGCN-main\TCGA\Lihc\lihc_new.pkl')
                        all_data = bls_tcga(all_data, train_data, val_data, test_data, patients)
                    elif cancer_type == 'kirc':
                        all_data = joblib.load('E:\Project\HGCN-main\TCGA\KIRC\kirc_new.pkl')
                        all_data = bls_tcga(all_data, train_data, val_data, test_data, patients)
                    elif cancer_type == 'luad':
                        all_data = joblib.load('E:\Project\HGCN-main\TCGA\LUAD\luad_new.pkl')
                        all_data = bls_tcga(all_data, train_data, val_data, test_data, patients)
                    elif cancer_type == 'ucec':
                        all_data = joblib.load(r'E:\Project\HGCN-main\TCGA\UCEC\ucec_new.pkl')
                        all_data = bls_tcga(all_data, train_data, val_data, test_data, patients)
            ...
            # torch.save(t_model.state_dict(), 'E:\Project\HGCN-main\output/'+sys_time.strftime('%Y-%m-%d')+label+'_'+str(seed)+'_'+str(n_fold)+'.pth')
            # torch.save(t_model.state_dict(), 'E:\Project\HGCN-main\output/'+sys_time.strftime('%Y-%m-%d')+'.pth')
    ...
    # ave=0
    # for i in all_fold_test_ci:
    #     for j in range(len(i)):
    #        ave+=all_fold_test_ci[i][j]
    #     print('ave:',ave)

    # joblib.dump(all_gnn_time,'E:\Project\HGCN-main\output/'+sys_time.strftime('%Y-%m-%d-%H-%M')+label+'.pkl')
    # joblib.dump(all_each_model_time,'E:\Project\HGCN-main\output/'+sys_time.strftime('%Y-%m-%d-%H-%M')+label+'.pkl')


def get_params():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_type", type=str, default="my", help="my/hgcn")
    parser.add_argument("--cancer_type", type=str, default="lihc", help="Cancer type:mydataset/mydataset1/lihc/kirc/luad/ucec")
