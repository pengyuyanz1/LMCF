...
from generators import conv_att
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # pos_emd_vis = expand_pos_embed[0][2].reshape(B, -1, C)
        pos_emd_mask = expand_pos_embed[mask].reshape(B, -1, C)
        # pos_emd_mask = expand_pos_embed[0][0:1].reshape(B, -1, C)
...
class fusion_model_mae_2(nn.Module):
    ...
    def forward(self, all_thing, train_use_type=None, use_type=None, in_mask=[], mix=False, ):
        ...
        if 'img' in data_type:
            # print(x_img.shape)
            # print(edge_index_img.shape)
            # x_img = self.mix(x_img)
            x_img = self.img_gnn_2(x_img, edge_index_img)
            # img_gat = GAT(1024, 512, x_img.shape[0], 0.5, 0.2, 3).to(device)
            # x_img = img_gat(x_img,edge_index_img)
            # print(x_img.shape)
        ...
        if 'rna' in data_type:
            # print(x_rna.shape)
            # x_rna = self.mix(x_rna)
            x_rna = self.rna_gnn_2(x_rna, edge_index_rna)
            # x_rna = self.rna_gat(x_rna,edge_index_rna)
            # print(x_rna.shape)
        ...
        if 'cli' in data_type:
            # print(x_cli.shape)
            # x_cli = self.mix(x_cli)
            x_cli = self.cli_gnn_2(x_cli, edge_index_cli)
            # x_cli = self.cli_gat(x_cli,edge_index_cli)
            # print(x_cli.shape)
        ...
        if len(train_use_type) > 1:
            ...
            if mix:
                # print('input:',mae_x.shape)
                # mlp mixer
                # mae_x = self.mix(mae_x)
                # mymodel
                mae_x = self.fusion(mae_x)
                # print('output:', mae_x.shape)
            ...
            if 'img' in train_use_type and 'img' in use_type:
                ...
                # print('x:',mae_x[train_use_type.index('img')].shape)
                k += 1
        ...
        multi_y = torch.empty((0)).to(device)
        if 'img' in data_type:
            ...
            multi_y = torch.cat((multi_y, torch.sigmoid(x_img)), 0)
        ...
        if 'rna' in data_type:
            ...
            multi_y = torch.cat((multi_y, torch.sigmoid(x_rna)), 0)
            k += 1
        ...
        if 'cli' in data_type:
            ...
            multi_y = torch.cat((multi_y, torch.sigmoid(x_cli)), 0)
            k += 1
        one_x = torch.mean(multi_x, dim=0)
        one_y = torch.mean(multi_y, dim=0)

        return (one_x, multi_x), save_fea, (att_2, att_3), fea_dict, (one_y,multi_y)
...
if __name__ == '__main__':
    ...
    print(model)
    input = joblib.load(r'E:\Project\HGCN-main\HGCN_code\all_data_hgcn_mydataset.pkl')
    mask = generate_mask(num=3)
    output = model(input['10269914'].to(device),['img', 'rna', 'cli'],['img', 'rna', 'cli'],mask,mix=True)
    print(output[-1])
